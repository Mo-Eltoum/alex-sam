...
from tools import alex_ingest_endpoint, ingest_financial_document
...
async def run_research_agent(topic: str = None) -> str:
    ...
    # )
    # model_name = os.environ.get("RESEARCHER_MODEL", "bedrock/amazon.nova-pro-v1:0")
    ...
    # MODEL = "bedrock/global.openai.gpt-oss-120b-1:0"
...
async def health():
    ...
    return {
        "service": "Alex Researcher",
        "status": "healthy",
        "alex_api_configured": bool(alex_ingest_endpoint() and os.getenv("ALEX_API_KEY")),
        "timestamp": datetime.now(UTC).isoformat(),
