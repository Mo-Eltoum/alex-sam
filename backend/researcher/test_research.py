...
def get_service_url():
    """HttpApi base URL. Researcher is POST /research on that API, not App Runner."""
    if os.getenv("LOCAL") == "True":
        return "http://localhost:8000"

    url = os.getenv("HTTP_API_URL") or os.getenv("RESEARCH_URL")
    if not url:
        region = os.getenv("DEFAULT_AWS_REGION", "eu-west-1")
        try:
            url = boto3.client("ssm", region_name=region).get_parameter(
                Name="/alex/http_api_url"
            )["Parameter"]["Value"]
        except Exception as e:
            print(f"❌ Error getting researcher URL: {e}")
            print("   SSM /alex/http_api_url is written by sam deploy. Set HTTP_API_URL in .env or deploy SAM first.")
            sys.exit(1)

    base = url.strip().rstrip("/")
    for suffix in ("/ingest", "/research", "/health"):
